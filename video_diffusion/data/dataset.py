...
class ImageSequenceDatasetMulti(Dataset):
    ...
    def get_frame_indices(self, index, folder_index):
        ...
        if self.sampling_rate<0:
            # random integer between  0 and -self.n_sample_frame
            sampling_rate=random.randint(1,-self.sampling_rate)
        else:
            sampling_rate=self.sampling_rate
        frame_start=index
        if frame_start >= n_images-self.n_sample_frame*sampling_rate:
            frame_start = n_images-self.n_sample_frame*sampling_rate
        
        # breakpoint()
        # Ensure frame indices do not exceed the number of images in the folder
        return [
            min(frame_start + i * sampling_rate, n_images - 1)  # Cap the index to the max available image
